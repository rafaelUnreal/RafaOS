...
uint32_t pci_read(pci_dev_t dev, uint32_t field) {
	...
	//volatile uint16_t *start_ptr = (uint16_t *)0xb8000;
	//*start_ptr++ = 0x57 << 8 | pci_size_map[PCI_BAR0];
//	char test[20];
	
	//itoa(test, pci_size_map[PCI_BAR0],16);
	//*start_ptr++ = 0x57 << 8 | test[0];
	//*start_ptr++ = 0x57 << 8 | test[1];
